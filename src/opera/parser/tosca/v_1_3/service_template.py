...
class ServiceTemplate_imports(Entity):
    # Implements the service template parser just for following imports
    ...
    def parse(cls, yaml_node, base_path, template_path, parsed_templates,
        ...
        service = super().parse(yaml_node)
        service.visit("prefix_path", template_path.parent)
        parsed, unresolved = service.get_imports(
            base_path, parsed_templates | set([template_path]),
            unresolved_imports
        )
        return service, parsed, unresolved

    def get_imports(self, base_path, parsed_templates, unresolved_imports):
        parsed = parsed_templates.copy()
        unresolved = unresolved_imports.copy()

        for import_def in self.data.get("imports", []):
            file_exists = import_def.file.resolve_path(base_path,
                                                       error_on_missing=False)
            import_path = import_def.file.data

            if not file_exists:
                unresolved = unresolved | set([import_path])
                continue
            ...
            with (base_path / import_path).open() as fd:
                yaml_data = yaml.load(fd, str(import_path))
            _, parsed, unresolved = ServiceTemplate_imports.parse(
                yaml_data, base_path, import_path, parsed, unresolved
            )
