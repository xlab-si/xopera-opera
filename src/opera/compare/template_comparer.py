...
class TemplateContext:  # noqa: R0903
...
class TemplateComparer:
    def compare_service_template(self, service_template1, service_template2, context):
        comparisons = {
            "nodes": MapComparison(self._compare_node),
        }
        return self._compare_item(service_template1, service_template2, comparisons, context)
    ...
    def _compare_node(self, node1, node2, context):
        comparisons = {
            ...
            "requirements": ListComparison(self._compare_requirement, self._get_name),
            "capabilities": ListComparison(self._compare_capability, self._get_name),
            "interfaces": MapComparison(self._compare_interface),
        }
        new_context = TemplateContext(node1, node2, context.workdir1, context.workdir2)
    ...
    def _compare_capability(self, capability1, capability2, context):
        ...
        return self._compare_item(capability1, capability2, comparisons, context)
    ...
    def _compare_relationship(self, rel1, rel2, context):
        ...
        new_context = TemplateContext(rel1, rel2, context.workdir1, context.workdir2)
        return self._compare_item(rel1, rel2, comparisons, new_context)
    ...
    def _compare_operation(self, operation1, operation2, context):
        comparisons = {
            ...
            "dependencies": ListComparison(self._compare_file, self._get_filename),
            "artifacts": ListComparison(self._compare_file, self._get_filename)
        }
        return self._compare_item(operation1, operation2, comparisons, context)

    @staticmethod
    def _compare_item(item1, item2, comparisons, context):
        diff = Diff()
        for name, comparison in comparisons.items():
            equal = True
            if isinstance(comparison, Comparison):
                equal, change = comparison.compare(getattr(item1, name), getattr(item2, name), context)
            if not equal:
    ...
    @staticmethod
    def _compare_types(types1, types2, context):
        type1 = types1[0]
        type2 = types2[0]
        return type1 == type2, [type1, type2]

    @staticmethod
    def _compare_capability_property(prop1, prop2, context):
        return prop1.data == prop2.data, [prop1.data, prop2.data]

    @staticmethod
    def _compare_none(attr1, attr2, context):
        # do not compare attribute values
        return True, ""

    @staticmethod
    def _compare_target(target1, target2, context):
