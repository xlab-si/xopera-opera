...
The following is a complete example of using the xOpera SaaS API through ``curl``::
    ...
    secret_base64="$(echo 'hello i am a secret!' | base64 -)"
    ...
    response="$(cookiecurl $api/secret)"
    redirect_url="$(echo $response | xmllint --html --xpath "string(//form[@id='kc-form-login']/@action)" - 2>/dev/null)"
    cookiecurl "$redirect_url" -d "username=$your_username" -d "password=$your_password" -d secretId=""

    # normal usage
    cookiecurl "$api/secret"
    cookiecurl "$api/secret" -XPOST -d "{\"name\": \"secret1\", \"path\": \"/tmp/secret.txt\", \"contents\": \"$secret_base64\"}"
    cookiecurl "$api/secret"
